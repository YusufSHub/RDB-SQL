/*==========================================================
					SQL SERVER - ÖNEMLÝ KODLAR-3

				(ALANLAR ÜZERÝNDE YAPILACAK YAPISAL ÝÞLEMLER)
============================================================
							INDEX
============================================================
	
1. BÝR TABLOYA ALAN EKLEME
2. BÝR TABLODAN ALAN ÇIKARTMA
3. BÝR ALANIN VERÝ TÝPÝNÝ DEÐÝÞTÝRMEK 
4. BÝR ALANA BÝR KISIT (CONSTRAINT) EKLEMEK
5. BÝR ALANDAN BÝR KISITI (CONSTRAINT) KALDIRMAK
6. BÝR ALANIN ADINI DEÐÝÞTÝRMEK
7. BÝR TABLONUN ADINI DEÐÝÞTÝRMEK*/
...
/*NOT: Eðer yeni eklenen alan otomatik artan sayý olacaksa, alan oluþturulurken yapýlmasý gerekir. 
Sonradan yapýlmasý için alanýn kaldýrýlmasý ve yeniden oluþturulmasý gerekir.

ÖRNEK: Ord_id alanýný otomatik artan sayý olacak þekilde, Order_date
alanýný date veri tipinde olacak þekilde oluþturmak için: */
...
--birden fazla sütun kaldýrma:
ALTER TABLE tablo_adi  DROP COLUMN sutun1, sutun2

-- ÖRNEK: Orders_dimen tablosundan Order_date alanýný çýkarmak için
...
ALTER TABLE tablo_adi DROP CONSTRAINT kýsýt_adý
...
-- ÖRNEK: Ord_id alanýndan UNIQUE kýsýtýný kaldýralým:
